...
fn is_air_extra_symbol(c: char) -> bool {
    matches!(
        c,
        '~' | '!'
            | '@'
...
fn is_valid_air_ident_char(c: char) -> bool {
    c.is_ascii_alphanumeric() || is_air_extra_symbol(c)
}
...
pub fn sanitize_to_air_ident(name: &str) -> String {
    ...
    let needs_encode = name.chars().any(|c| !is_valid_air_ident_char(c));
    if !needs_encode {
        return name.to_string();
    }
    res.push_str(MANGLE_PREFIX);
    for c in name.chars() {
        if is_valid_air_ident_char(c) {
            res.push(c);
        } else {
            res.push('$');
